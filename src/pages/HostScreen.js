  ...
  useEffect(() => {
    ...
    const handleHostNotification = ({ message }) => {
      toast.info(message, {
        ...
        closeOnClick: false,
        draggable: false,
        onClose: () => socket.emit("revealCards", { gameId }),
      });
    };
    ...
    // const handleGameReplayed = ({ newGameId, playerCodes, spectatorCode }) => {
    //   setRevealedCards([]);
    //   setEnlargedCard(null);
    //   setMessages([]);
    //   setShowLeaderboard(false);
    //   toast.success("New game started! Share new invite codes.");
